...
import { SkillResponse, SkillRequest } from '../../../core/models/skill.model';
  ...
  template: `
    <div class="skills-container">
      <div class="header">
        ...
        <button class="btn btn-primary" (click)="showCreateForm = true">Create New Skill</button>
      </div>

      <div *ngIf="showCreateForm || editingSkill" class="form-container">
        <h2>{{ editingSkill ? 'Edit Skill' : 'Create New Skill' }}</h2>
        <form [formGroup]="skillForm" (ngSubmit)="onSubmit()">
          <div class="form-group">
            <label>Title *</label>
            <input type="text" formControlName="title" />
            <div *ngIf="skillForm.get('title')?.invalid && skillForm.get('title')?.touched" class="error">
              Title is required
            </div>
          </div>
          ...
          <div class="form-actions">
            <button type="submit" class="btn btn-primary" [disabled]="skillForm.invalid || loading">
      ...
      <div *ngIf="loading && !showCreateForm" class="loading">Loading skills...</div>
        ...
        <table>
          ...
          <tbody>
            <tr *ngFor="let skill of skills">
              ...
              <td>{{ skill.slug }}</td>
              <td>
                <span [class]="skill.published ? 'badge published' : 'badge draft'">
                  {{ skill.published ? 'Published' : 'Draft' }}
                </span>
              </td>
              <td>{{ skill.updatedAt | date:'short' }}</td>
              <td>
                <button class="btn btn-sm btn-primary" (click)="editSkill(skill)">Edit</button>
...
export class SkillsComponent implements OnInit {
  ...
  editingSkill: SkillResponse | null = null;
  skillForm: FormGroup;
  currentPage = 0;
  totalPages = 0;
  ...
  loadSkills(page: number = 0): void {
    this.loading = true;
    this.error = '';
    this.skillService.listSkills(page, 20).subscribe({
      next: (response) => {
  ...
  onSubmit(): void {
    if (this.skillForm.valid) {
      ...
      if (this.editingSkill) {
        this.skillService.updateSkill(this.editingSkill.skillKey, request).subscribe({
          next: () => {
            this.loading = false;
            this.cancelForm();
  ...
  deleteSkill(skill: SkillResponse): void {
    if (confirm(`Delete "${skill.title}"?`)) {
      this.loading = true;
      this.skillService.deleteSkill(skill.skillKey).subscribe({
        next: () => {
          this.loading = false;
          this.loadSkills(this.currentPage);
        },
  ...
  cancelForm(): void {
    ...
    this.skillForm.reset({ published: false });
  }
}
