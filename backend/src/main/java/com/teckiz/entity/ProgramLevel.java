...
public class ProgramLevel {
    ...
    @Column(name = "level_key", length = 255)
    ...
    @ManyToOne(fetch = FetchType.LAZY)
    @JoinColumn(name = "program_level_type_id")
    private ProgramLevelType programLevelType;
    ...
    protected void onCreate() {
        if (levelKey == null) {
            levelKey = UtilHelper.generateEntityKey();
        }
    }
}
